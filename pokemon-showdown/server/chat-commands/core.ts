...
export const commands: Chat.ChatCommands = {
	...
	msg(target, room, user) {
		...
		if (targetUsername === '~') {
			...
			// Upstream also short-circuits a COMMAND straight to /offlinemsg here (`isCommand ||`
			// in the condition below). This fork does not support commands in offline PMs at all -
			// /offlinemsg rejects them outright - so that half of upstream's change is deliberately
			// not taken, and msghelp keeps its single line for the same reason.
	...
	infinitesubmit(target, room, user) {
		room = this.requireRoom();
		if (!room.battle) throw new Chat.ErrorMessage(this.TL`This isn't a battle room.`);
	...
	async savereplay(target, room, user, connection) {
		if (!room?.battle) throw new Chat.ErrorMessage(this.TL`You can only save replays for battles.`);
	...
	cancelchallenge(target, room, user, connection) {
		...
		if (!this.pmTarget) return this.popupReply(this.TL`User "${targetUsername}" not found.`);
