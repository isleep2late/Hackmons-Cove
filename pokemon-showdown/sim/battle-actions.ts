...
export class BattleActions {
	...
	applyRecoilDamage(damageDealt: number, move: Move, pokemon: Pokemon): number | null {
		...
		if (move.struggleRecoil) {
			recoilDamage = this.dex.gen >= 5 ?
				this.battle.clampIntRange(Math.round(pokemon.baseMaxhp / 4), 1) :
				this.battle.clampIntRange(this.battle.trunc(pokemon.maxhp / 4), 1);
		} else if (move.mindBlownRecoil) {
			recoilDamage = Math.round(pokemon.maxhp / 2);
		} else if (move.recoil || move.id === 'chloroblast') {
			recoilDamage = this.calcRecoilDamage(damageDealt, move, pokemon);
		} else {
			return null;
		}
