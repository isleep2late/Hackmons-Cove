	...
	for (; x < rows.length && findSpecies(rows[x]).offset === undefined; x++) {
		...
		case 'IVs':
			for (j = 1; j < currentRow.length; j++) {
				...
				if (gen === 1 || gen === 2 || gen === 11) dvs[currentIV[1]] = Math.floor(parseInt(currentIV[0]) / 2);
				else ivs[currentIV[1]] = parseInt(currentIV[0]);
			}
			if (gen === 1 || gen === 2 || gen === 11) currentPoke.dvs = dvs;
