...
var LEGACY_STATS = [LEGACY_STATS_GSC, LEGACY_STATS_RBY, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC, LEGACY_STATS_GSC];
	...
	if (m) {
		...
		if (actual.type !== m[1]) {
			...
			if (hpIVs && (gen < 7 || gen === 10)) {
				...
				pokeObj.change();
				moveGroupObj.children(".move-bp").val(gen >= 6 && gen !== 10 && gen !== 11 ? 60 : 70);
			}
		} else {
			moveGroupObj.children(".move-bp").val(actual.power);
		}
	} else if (((gen >= 2 && gen <= 6) || gen === 10 || gen === 11) && HIDDEN_POWER_REGEX.test($(this).attr('data-prev'))) {
...
$(".set-selector").change(function () {
	...
	if (pokemon) {
		...
		if (randset) {
			...
			if (gen >= 3 && gen !== 11) $(this).closest('.poke-info').find(".ability-pool").show();
			...
			if (!(gen === 8 && setName === 'BDSP Randoms') && gen !== 1) {
				...
				if (gen >= 9 && gen !== 11) $(this).closest('.poke-info').find(".tera-type-pool").show();
			} else {
		...
		if (regSets || randset) {
			...
			var evsDefault = gen < 3 || gen === 11 ? 252 : ($("#randoms").prop("checked") ? 84 : 0);
			for (i = 0; i < LEGACY_STATS[gen].length; i++) {
			...
			for (i = 0; i < LEGACY_STATS[gen].length; i++) {
				...
				pokeObj.find("." + LEGACY_STATS[gen][i] + " .evs").val(gen > 2 && gen !== 11 ? 0 : 252);
		...
		if ((gen === 8 || gen === 10) && pokemon.canGigantamax) {
...
function correctHiddenPower(pokemon) {
	...
	if (gen >= 7 && gen !== 11 && (!pokemon.level || pokemon.level >= 100)) return pokemon;
...
function createPokemon(pokeInfo) {
	if (typeof pokeInfo === "string") { // in this case, pokeInfo is the id of an individual setOptions value whose moveset's tier matches the selected tier(s)
		...
		for (var i = 0; i < LEGACY_STATS[gen].length; i++) {
			...
			ivs[stat] = (gen >= 3 && gen !== 11 && set.ivs && typeof set.ivs[legacyStat] !== "undefined") ? set.ivs[legacyStat] : 31;
			var sps = set.sps;
			if (isChampions) {
				var sps = 0;
				if (set.sps) {
		...
		for (var i = 0; i < LEGACY_STATS[gen].length; i++) {
			...
			ivs[stat] =
				gen == 0 ? 31 : gen > 2 && gen !== 11 ? ~~pokeInfo.find("." + LEGACY_STATS[gen][i] + " .ivs").val() : ~~pokeInfo.find("." + LEGACY_STATS[gen][i] + " .dvs").val() * 2 + 1;
...
function getMoveDetails(moveInfo, opts) {
	...
	if (gen >= 4 && gen !== 11) overrides.category = moveInfo.find(".move-cat").val();
...
function createField() {
	...
	var isBatonBoost = [$("#batonBoostL").prop("checked"), $("#batonBoostR").prop("checked")];
...
var GENERATION = {
	...
	'8': 8, 'ss': 8,
	'9': 9, 'sv': 9,
	'10': 10, 'nn': 10, 'nonerfs': 10, 'phnn': 10,
	'11': 11, 'sw': 11, 'sw97': 11, 'spaceworld': 11
};
...
var SETDEX = [
	...
	typeof SETDEX_NN === 'undefined' ? {} : SETDEX_NN,
	typeof SETDEX_SW === 'undefined' ? {} : SETDEX_SW,
];
...
$(".gen").change(function () {
	...
	var firstSetOption = getFirstValidSetOption() || getSetOptions()[1];
	if (firstSetOption) $(".set-selector").val(firstSetOption.id);
	$(".set-selector").change();
});
...
function loadDefaultLists() {
	$(".set-selector").select2({
		...
		initSelection: function (element, callback) {
			callback(getFirstValidSetOption() || getSetOptions()[1]);
		}
	});
}
...
$(document).ready(function () {
	...
	if ($("#champions").prop("checked")) {
		...
		var firstChampionsOption = getFirstValidSetOption() || getSetOptions()[1];
		if (firstChampionsOption) $(".set-selector").val(firstChampionsOption.id);
		$(".set-selector").change();
	}
	...
	// Ensure the first calculation runs even when booting into a ?gen= param
