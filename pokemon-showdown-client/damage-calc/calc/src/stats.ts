...
const NN: StatID[] = GSC; // No Nerfs: all six stats, modern formula
const SW: StatID[] = GSC; // SpaceWorld '97: all six stats, GSC formula

export const STATS: Array<Array<StatID | 'spc'> | StatID[]> =
  [[], RBY, GSC, ADV, DPP, BW, XY, SM, SS, SV, NN, SW];

type HPTypeName = Exclude<TypeName, 'Normal' | 'Fairy' | 'Stellar' | 'Shadow' | 'Bird' | '???'>;
...
export const Stats = new (class {
  ...
  getHiddenPowerIVs(gen: Generation, hpType: HPTypeName) {
    ...
    return gen.num === 2 || gen.num === 11 ? Stats.DVsToIVs(hp.dvs) : hp.ivs;
  }
