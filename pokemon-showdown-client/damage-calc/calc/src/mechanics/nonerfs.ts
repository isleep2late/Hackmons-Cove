...
export function calculateNoNerfs(
  ...
  if (move.originalName === 'Weather Ball') {
    ...
    desc.moveType = type;
  } else if (move.named('Judgment') && attacker.hasItem('Legend Plate')) {
    type = getBestJudgmentType(gen, defender);
    desc.moveType = type;
  ...
  const noTypeChange = move.named(
    ...
    'Terrain Pulse',
    'Struggle',
  ) || (move.named('Tera Blast') && attacker.teraType && attacker.teraType !== 'Normal');
  ...
  if (move.named('Sheer Cold', 'Fissure', 'Horn Drill', 'Guillotine')) {
    result.damage = defender.isDynamaxed ? 0 : defender.maxHP();
    return result;
  }
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateNoNerfs(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateBasePowerSMSSSV(
  ...
  basePower = NN_OF(Math.max(1, pokeRound((basePower * chainMods(bpMods, 41, 2097152)) / 4096)));
  if (
    attacker.teraType &&
...
export function calculateBPModsSMSSSV(
  ...
  const terrainMultiplier = 6144; // phnn restores the 1.5x terrain boost
  ...
  if (attacker.hasItem(`${move.type} Gem`)) {
    ...
    bpMods.push(4505);
    desc.attackerItem = attacker.item;
  } else if (attacker.hasItem('Pink Bow', 'Polkadot Bow') && move.hasType('Normal')) {
    bpMods.push(4505);
    desc.attackerItem = attacker.item;
...
export function calculateAttackSMSSSV(
  ...
  attack = NN_OF(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}
...
export function calculateAtModsSMSSSV(
  ...
  if (attacker.isWildMight) {
    atMods.push(8192);
    desc.attackerAbility = 'Wild Might' as AbilityName;
  }
...
export function calculateDefenseSMSSSV(
  ...
  return NN_OF(Math.max(1, pokeRound((defense * chainMods(dfMods, 410, 131072)) / 4096)));
}
...
export function calculateDfModsSMSSSV(
  ...
  if (defender.isWildMight) {
    dfMods.push(8192);
    desc.defenderAbility = 'Wild Might' as AbilityName;
  }

  if ((defender.hasItem('Eviolite') &&
      (defender.name === 'Dipplin' || gen.species.get(toID(defender.name))?.nfe ||
       NN_EVIOLITE.includes(toID(defender.name) as string))) ||
...
function calculateBaseDamageSMSSSV(
  ...
  const isSpread = field.gameType !== 'Singles' &&
     ['allAdjacent', 'allAdjacentFoes'].includes(move.target);
  if (isSpread) {
    baseDamage = pokeRound(NN_OF(baseDamage * 3072) / 4096);
  }
  ...
  if (
    ...
    baseDamage = pokeRound(NN_OF(baseDamage * 6144) / 4096);
    isMegaSol ? desc.attackerAbility = attacker.ability : desc.weather = field.weather;
  ...
  if (isCritical) {
    baseDamage = Math.floor(NN_OF(baseDamage * 1.5));
...
function getBestJudgmentType(gen: Generation, defender: Pokemon): TypeName {
  ...
  const eff = (atk: TypeName, def: TypeName) =>
    gen.types.get(toID(atk))?.effectiveness[def];
  let best: TypeName[] = ['Normal'];
  ...
  for (const dt of defender.types) {
    ...
    for (const t of best) {
      const e = eff(dt, t);
      const rank = e === 0 ? 3 : e === 0.5 ? 1 : e === 2 ? -1 : 0;
      if (rank > bestRank) {
        bestRank = rank;
        keep = [t];
