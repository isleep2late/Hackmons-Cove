      ...
      presetlessIndices.forEach((pokemonIndex) => {
        ...
        if (!preset?.calcdexId) {
          ...
          if (pokemon.phnnMinConfusion) {
            ...
            // same as applyPreset(): remember the nature being displaced so the flag is reversible
            const displaced = pokemon.nature;
          
            pokemon.nature = getPhnnMinConfusionNature(displaced, PokemonNatureBoosts);
            if (displaced) {
              pokemon.phnnPrevNature = displaced;
            }
          }
